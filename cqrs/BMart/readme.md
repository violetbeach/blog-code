...
B마트에서는 이벤트 소싱 방식으로 명령 모델에서 데이터를 변경하면 이벤트를 날리고, 해당 이벤트를 구독하던 애플리케이션에서 조회모델을 생성한다.
...
B마트에서 가장 복잡한 조회 모델은 카탈로그 모델이라고 한다. 그래서 요구사항들을 만족하기 위해 카탈로그 조회 모델로 아래의 **전체 트리 모델 하나**만 사용하면 될 것이라고 판단했다.
...
추가로 지점 수와 카탈로그 수와 상품 수와 비례해서 모델의 크기가 커지는 구조였고, 사용자 수가 늘면 트래픽을 감당할 수 없는 상황이 되었다.
