...
- 메인 코드가 방대해진다.
- 비 관심사(조회 모델 생성)가 **주 관심사(API 기능)와 성능 및 장애를 공유**한다.
...
프로젝트에서는 영상 플랫폼이라 **조회 모델 생성이 동시에 이뤄지지 않아도 크게 상관이 없었다.**

그래서 IPC 방식 중 Sync(HTTP, gRPC, ...) 방식이 아닌 Async(Message Queue) 방식을 사용해서 이벤트를 발행하고 **Consumer에서 조회 모델을 생성**하도록 설계했다.

**Consumer 모듈도 별도로 분리**했다.
...
CircleCI에서는 많이 복잡했다. 아래 포스팅에서 상세하게 정리했으니 참고하자. (삽질을 많이했다.)
