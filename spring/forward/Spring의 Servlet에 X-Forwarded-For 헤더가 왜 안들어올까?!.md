아래는 실무 중에 만난 이슈와 해결 과정에 대해 다룹니다.

포스팅이 **정확한 원인**과 **해결 방법**보다는 **풀어나간 과정**에 집중되어 있는 점 참고 부탁드립니다!
- 원인과 해결 방안이 궁금한 분은 가장 아래로 내리시면 될 것 같습니다!

## 헤더에 X-Forwarded-For이 안들어오는 문제 발견

아래는 사내 전체에서 쓰는 공통 라이브러리의 코드를 재작성한 것이다.
...
팀 동료한테 이슈를 알리고, 타 팀 동료에게 여쭤봤는데 다음의 대답이 돌아왔다. 

![img_1.png](img_1.png)

왜 안되는 지는 모르지만, 다른 방법으로 Ip를 추출하고 있었다고 한다.

그래서 '다른 사람들이 겪을 불편을 겪기 전에 내가 해결하자!' 라고 생각을 하게 되었다.

![img_2.png](img_2.png)

서버에 위와 같은 헤더가 들어오고 있어서 그거에 맞게 헤더만 수정하면 되었지만, 운영 팀에 정책을 확인해본 결과 `X-Forwarded-For`를 사용해야 한다고 하셨다.
...
- 리버스 프록시에서 헤더가 정말 나온 것이 맞는 지 의심이 커져감
  - 엔지니어 분께 보여달라고 요청 드려봤으나 다른 프로젝트(Node, PHP)도 설정이 동일한데 잘 나온다고만 말씀해주셨다. (확인이 어려운가 보다..)
...
그러던 중 **정말 중요한 단서**를 하나 찾았다!

#### Local에서는 잘 되는데, k8s 환경에서는 안되는 현상
...
그 결과 PostMan에서 요청을 보냈을 때 처럼 헤더가 잘 나왔다.
...
`X-Forwarded-For`만 사라진 것을 확인할 수 있었다!
...
`Valve`를 거치기 전에는 Header에 값이 있다가, **Valve를 거친 후에는 Header의 값을 삭제**한다고 한다!!! 해당 헤더는 **request.remoteAddr에 저장한다.**
...
로컬에서는 클라우드 플랫폼이 아니므로 `false`가 되어서 `Valve`가 동작하지 않아서 정상 동작하지 않을 수 있었던 것이다.

## 처리

결국 내가 예상은 했지만 아닐 것이라고 믿었던 **ServletContainer(Tomcat)**가 문제였고, 거기서 헤더를 지우는 것이 맞았다.

해결은 코드의 `request.getHeader("X-Forwarded-For")`를 `request.getAddr()`로 바꿔서 비교적 간단하게 해결할 수 있었다.
