...
해당 포스팅에서는 IntelliJ의 기능을 사용해서 직접 의존성을 검사한다.
...
연관 관계의 종류는 대표적으로 객체 참조 방식과 ID 참조 방식이 있다. 여기서 객체 참조 방식은 연관 관계 중 **가장 높은 결합도**를 가진다.

객체 참조는 아래의 문제가 생길 수 있다.
- 도메인 간의 결합도 상승
...
Cover 바운디드 컨텍스트의 `CoverService`에서는 Comment 바운디드 컨텍스트의 `CoverCommentService`에 의존하고 있다. Comment 바운디드 컨텍스트에서도 `Comment` 도메인 모델도 Cover 바운디드 컨텍스트의 `Cover` 도메인 모델에 의존하고 있다.

의존성 사이클이 돌면 코드 수정의 범위를 파악과 코드 관리가 어려워진다. 그래서 동일한 패키지에 둘 지 의존성 사이클을 끊을 지 고민했다.

결과 Cover와 Comment가 **다른 바운디드 컨텍스트라고 판단**했기 때문에 의존성 사이클을 끊어야 했다.
...
- 실시간으로 같이 생성되고 삭제되는 것이 아니기에 가능한 분리되어야 한다.

그래서 다른 바운디드 컨텍스트 간 도메인 참조는 **객체 참조**가 아닌 **ID 참조**를 사용하도록 처리했다.
...
결과적으로 Comment 도메인 모델은 Cover 도메인 모델에 의존하지 않게 되었다.

### 2. 이벤트 전파

수정 후에도 Cover와 Comment 바운디드 컨텍스트 간 패키지 의존성 사이클이 존재했는데 아래 코드 때문이었다.
...
`CommentService`는 Cover의 존재 여부 등 검증을 위해 **Cover 바운디드 컨텍스트**의 `CoverService`에 의존한다. `CoverService`는 Comment도 함께 삭제하기 위해 **Comment 바운디드 컨텍스트**에 의존한다.

하나의 의존성은 끊어야겠다고 판단을 했고 **CommentService에서 Cover 존재 여부를 파악하는 것은 적합한 의존성**이지만, **CoverService에서 Comment를 삭제하는 것은 강한 일관성이 필요한 부분이 아니**라고 생각했다.
...
이제 더 이상 Cover 바운디드 컨텍스트는 Comment 바운디드 컨텍스트에 의존하지 않는다.

### 3. 중간 객체 / DIP 활용

Song 바운디드 컨텍스트는 Artist 존재 여부 확인을 위해 Artist 바운디드 컨텍스트에 의존하고 있는 상황이다.

그런데 ArtistService에서도 해당 가수의 음악 장르를 알아야 한다. MusicGenre는 Song 바운디드 컨텍스트에 존재한다.
...
바운디드 컨텍스트 간 의존성 사이클은 의도했던 대로 잘 끊어졌다.

그런데 **user 패키지와 user.exception 패키지 간의 의존성 사이클**이 돌고 있다. 그래서 아래의 의문이 생겼다.

_**바운디드 컨텍스트 내부에서도 패키지 의존성 사이클을 끊어야 하나?!**_

**끊어야 한다!** 두 패키지 간의 의존성 사이클이 돈다는 것은 **user 패키지와 user.exception 패키지**가 **함께 변경됨**을 의미한다.
...
사실 전자도 나쁘지는 않다고 생각했지만 Domain에서 터지는 Exception이 있고 Service(Business)에서 터지는 Exception도 있었다. Business Exception을 Domain으로 포함 시키는 것을 원하지 않았다.

그래서 후자를 택했다.

## 의존 끊기

해당 의존성 사이클에서는 2가지의 의존이 존재한다.
...
OOP의 객체 자율화에 의하면 모든 도메인은 자신의 상태를 검증해야 한다. Exception을 발생하는 것은 맞아보인다.

도메인 모델에서 IllegalStateException을 사용하면 내 패키지의 exception에는 의존하지 않을 수 있다. 하지만 IllegalStateException 등 자바에서 제공하는 Exception을 사용하면 아래의 문제가 있었다.

- 예외를 잡아서 처리하기가 곤란하다.
   - 사용자에게 예외 메시지 전달을 위해 Service Layer에서 **처리(try-catch)**를 해야 한다.
   - 코드가 지저분해질 수 있다.
- 패키지만 다를 뿐 IllegalStateException에 의존하게 되므로 아키텍처 개선으로 볼 수 없다.

그래서 **Domain에서 Exception에 의존하는 구조는 유지**하기로 했다.

#### Exception에서 Domain 의존

의존을 끊을 지 여부를 판단하기 전에 먼저 의존에 대해서 알 필요가 있다.

**A가 B를 의존한다는 것**은 **B가 변경되었을 때 A도 변경된다는 것**을 의미한다. 그래서 **Domain이 변경되었을 때 Exception도 변경되어야 하는가?**를 고민하면 된다. Domain은 가장 Core한 부분이기 때문에 Exception에 영향이 가도 크게 상관은 없었던 것 같다.

그렇지만 의존성 사이클을 끊고 싶어서 아래와 같이 처리했다.
