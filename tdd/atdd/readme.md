...
최근 TDD에 큰 관심이 생겨서 요즘 핫하다는 ATDD에 대해, 그리고 기본적인 TDD에 대해 더 깊게 이해하려고 마음을 먹었다.

NEXTSTEP에 ATDD, 클린 코드 with Spring이라는 좋은 강의 코스가 있는데, 가격이 80만원이다... ㄷㄷ
- https://edu.nextstep.camp/c/R89PYi5H/

사실 조금 부담스러워서 우아한 ATDD라는 우아한 테크 세미나 발표가 있다.
- 아래는 해당 발표 내용을 공부하며 정리한 것이다.. ㅋㅋ
...
RANDOM_PORT를 사용하면서 트랜잭션이 롤백되지 않는다.
- 호출되는 쪽은 전혀 다른 스레드에서 새로운 트랜잭션으로 커밋하기 때문

이를 해결하려면 매 테스트마다 데이터를 초기화해야 한다.

### 초기화

초기화의 경우 아래의 방법을 고민할 수 있다.
...
1. @DirtiesContext
    - 완전히 테스트가 격리되서 독립적으로 실행된다는 장점이 있음
    - 매번 Context를 새로 구성하다보니 시간이 많이 걸림 (치명적인 단점)
2. EntityManager를 활용
    - 각 테이블 Truncate 실행
    - AutoIncrement 초기화
...
강연에서는 실제 요청과, 가짜 요청을 혼합해서 사용한다고 한다.
- Github API의 경우 테스트 계정으로 실제 요청
  - 외부 서비스에 의존적인 테스트가 된다는 단점이이 있음
  - 메인 서비스에서 매우 중요한 기능이라서 실제 요청을 사용한다고 한다.
- 결제 API 연동
...
- Top-Down 방향으로 큰 범위부터 테스트를 작성한다.
- 
