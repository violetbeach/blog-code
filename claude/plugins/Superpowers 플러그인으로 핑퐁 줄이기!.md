...
Superpowers 플러그인을 사용해서 클로드에게 작업을 맡기면 바로 손부터 나가지 않고(개발부터 하지 않고), 충분한 계획과 설계를 가지고 일을 하기에 핑퐁을 반복하지 않아도 꽤 만족스러운 결과물을 가져온다.

다만, 작업 볼륨에 비해 작은 범위의 내용만 전달하면 계획 과정에서의 핑퐁은 자주 있을 수 있다. 그 부분은 클로드가 작업한 후 우리가 발견해서 대격변을 하는 것이 아니므로, 토큰 사용량도 아끼고 스트레스도 덜 받을 수 있다. SDD와 TDD를 할 때 슈퍼파워를 사용하면 특히 유용하다. 
