...
- 사실 뭐 어렴풋이 **도메인 이벤트(내부 이벤트)**와 **메시징 시스템(외부 이벤트)**를 사용해서 **Publisher와 Subscriber 형태로 구축**하면 되겠지..? 라고 생각했지만, 실무자의 경험이 듣고 싶었다.

그래서 해당 포스팅에서는 **권용근님의 회원시스템 이벤트기반 아키텍처 구축하기**라는 강연을 보고 나름대로 정리하는 것을 목표로 한다.
...
즉 1. 언제(시간), 2. 누가(식별자), 3. 무엇을(행위), 4. 어떤 변화가(속성)을 담는다.

단, 이를 그대로 DB 컬럼에 옮길 경우 이벤트가 외부로 발행되지 않고 DB에만 삽입된 채 유실되는 문제가 발생할 수 있다.

![img_7.png](img_7.png)

그래서 추가로 발행 여부를 상태로 보관하면서 이벤트 유실을 방지할 수 있다.

즉, 이벤트를 생성할 때는 published 컬럼을 false로 생성하고 이벤트 발행을 할 때 published를 true로 변경한다는 정책을 만듬으로써

Batch, Scheduler, ... 에서 일정 주기별로 미발행된 이벤트에 대해서 발행하는 처리를 할 수 있다.

결과적으로 이벤트 저장소를 통해 이벤트의 유실을 막으면서도 느슨한 결합을 유지할 수 있었고,

해당 event에 아래의 컬럼들을 추가했고, 그 결과 event는 도메인의 주요 행위가 모두 기록되고 있었다. 결국, 로그 테이블 또한 이벤트 저장소로 통합할 수 있었다.
```sql
