...
이제 모든 메일과 메일함을 매칭시키는 과정이 필요하지 않게 되었다. 메일의 메일함 컬럼에서 잠기지 않은 메일함 목록을 `IN`절로 비교만 해주면 되었다.
...
> 해당 계정은 **메일을 수백만 개 가지고 있는 테스트 계정**이며, 나중에 알게 되었지만 메일을 1만 개 정도 가진 계정의 경우 TPS가 780 가량 나왔다.
